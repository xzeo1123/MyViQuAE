...
from meerqat.models import mm, qa, rr
...
def map_kilt_triviaqa():
    kilt_tasks = load_dataset("kilt_tasks", "triviaqa_support_only")
    ...
    for k in ['train', 'validation', 'test']:
        ...
        filtered = kilt_tasks[k].filter(lambda x: x['id'] in triviaqa_map)

        mapped = filtered.map(
            add_missing_data,
            fn_kwargs={'trivia_qa_subset': trivia_qa[k], 'triviaqa_map': triviaqa_map}
        )

        new_kilt_tasks[k + '_triviaqa'] = mapped

    return new_kilt_tasks
